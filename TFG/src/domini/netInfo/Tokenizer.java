package domini.netInfo;

/**
 * This class perform similar functionality to StringTokenizer class but faster.
 * The difference is StringTokenizer doesn't count empty token, but this class
 * does.
 * 
 * @author Henry Zheng
...
public class Tokenizer {
    ...
    /**
     * It's different from the other parse method in that it checks left and
     * right string first, which take higer priority than the delimiter. For
     * example, if left and right is ", for string a:b:1"c:d"2:3 , it returns {
     * a, b, 1"c:d"2, 3 }
     * 
     * @param left
     *            the openning tag of higher priority token
     * @param right
     *            the closing tag of higher priority token
    ...
    public static String[] parse(String text, char delimiter,
	    boolean trimEachToken, String left, String right) {
	if (text == null)
	    return null;
     ...
     */
    public static String[] parse(String text, char delimiter,
	    boolean trimEachToken, boolean ignoreEmptyToken) {
	if (text == null)
	    return null;
	StringArray tokens = new StringArray();
	int pos1 = -1;
